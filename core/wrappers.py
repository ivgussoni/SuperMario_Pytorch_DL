...
class FrameBuffer(ObservationWrapper):
    def __init__(self, env, num_steps, dtype=np.float32):
        ...
        self._dtype = dtype
        self.observation_space = Box(obs_space.low.repeat(num_steps, axis=0), obs_space.high.repeat(num_steps, axis=0),
                                     dtype=self._dtype)
...
def wrap_environment(environment, action_space, model_patch=' ', monitor=False):
    env = gym_super_mario_bros.make(environment)
    if monitor:
        env = wrappers.Monitor(env, model_patch, force=True)

    if action_space == 'complex' or len(action_space) == 12:
        env = JoypadSpace(env, COMPLEX_MOVEMENT)
    if action_space == 'simple' or len(action_space) == 7:
        env = JoypadSpace(env, SIMPLE_MOVEMENT)
    if action_space == 'right-only' or len(action_space) == 5:
